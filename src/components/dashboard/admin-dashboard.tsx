  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <Stat label="Company share" value={money(f.companyShare)} hint="After teacher commissions" tone="success" icon={<Building2 className="h-5 w-5" />} />
        <Stat label="Owed to teachers" value={money(f.teacherBalanceOwed)} hint={`${money(f.teacherShare)} earned, ${money(f.totalPayouts)} paid`} tone="warning" icon={<HandCoins className="h-5 w-5" />} />
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          <CardHeader title="Monthly collections" description="Fee payments received, split into company share and teacher share." action={<Link href="/finance" className="text-xs font-medium text-brand-700 hover:underline">Company account</Link>} />
          <div className="p-5">
            <MonthlyBars data={data.monthly} series={[{ key: "companyShare", label: "Company share", color: CHART_COLORS.company }, { key: "teacherShare", label: "Teacher share", color: CHART_COLORS.teacher }]} />
          </div>
        </Card>

        <Card>
          <CardHeader title="Teachers" action={<Link href="/teachers" className="text-xs font-medium text-brand-700 hover:underline">View all</Link>} />
          <div className="divide-y divide-slate-100">
            {data.teachers.length === 0 && <EmptyState title="No teachers yet" />}
            {data.teachers.map((t) => (
              <Link key={t.id} href={`/teachers/${t.id}`} className="block px-5 py-3 hover:bg-slate-50">
                <div className="flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    <p className="truncate font-medium text-slate-900">{t.name}</p>
                    <p className="truncate text-xs text-slate-500">{t.subjects.join(", ") || "No subjects"} · {percent(t.defaultCommissionPercent)} · {t.totals.studentCount} students</p>
                  </div>
                  <div className="text-right">
                    <p className={`text-sm font-semibold ${t.totals.balance < 0 ? "text-rose-600" : "text-brand-700"}`}>{money(t.totals.balance)}</p>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          ...
          {data.recentPayments.length === 0 ? <EmptyState title="No payments yet" /> : (
            <Table>
              <THead><TR><TH>Date</TH><TH>Student</TH><TH className="text-right">Amount</TH><TH className="text-right">Teacher share</TH></TR></THead>
              <TBody>
                {data.recentPayments.map((p) => (
                  <TR key={p.id}>
                    <TD className="whitespace-nowrap">{date(p.paidAt)}</TD>
                    <TD><Link href={`/students/${p.studentId}`} className="font-medium text-slate-900 hover:text-brand-700">{p.student?.name}</Link><p className="text-xs text-slate-500">{p.teacher?.user.name}</p></TD>
